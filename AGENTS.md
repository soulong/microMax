...
package does, how they depend on each other, what every module's interface is,
and where the seams live.
...
`Pillow`, `scikit-image`, `scipy`, `pyyaml`, `albumentationsx`, `torch`,
`timm`, `cellpose`, `jax`, `PySide6`, `matplotlib`, `flask`, `umap-learn`,
`pygwalker`, `pydantic`, `scikit-learn`, `opencv-python`, `tqdm`, `natsort`,
`openpyxl`, `lightly`.
...
- The three consumers never import each other directly. `microModel` is
  self-contained — its model/dataset helpers (`build_backbone`, `FocalLoss`,
  `ClassificationHead`, `Model`, the datasets, utils) are originals owned by
  `microModel` (microBase has no torch code); only `load_yaml`,
...
Per §3.16, `session.yml` is read once on first Browse of a directory;
action buttons write current GUI state. Writers below are action buttons
(including "Load Dataset", which is distinct from "Browse").

| Section | Writer (action buttons) | Reader (first Browse only) |
|---|---|---|
| `image_pattern`, `mask_pattern`, `image_subdir_pattern` | microProfiler (Load Dataset via `set_patterns`, Run/Apply via `_save_session_yml`), microVis (Load Dataset, Export, Write-to-DB) | microProfiler, microVis |
| `applied_steps` | microProfiler (Run/Apply via `_save_session_yml`) | microProfiler (skip logic) |
| `channel_colors` (`{ch: {color, vmin, vmax}}`) | microVis (Load Dataset, Export, Write-to-DB, Auto, ImageControls Reset) | microVis |
| step params (`resize`, `basic`, `zproject`, `tile`, `segment`, `image_profile`, `object_profile`) | microProfiler (Run/Apply via `_save_session_yml`) | microProfiler |
| `filter` | microProfiler (Run/Apply via `_save_session_yml`) | microProfiler |

---

## 3. Cross-cutting design principles

1. **Minimal interface, deep module.** `microBase` exposes 22 public names from
   a flat 10-module layout. Consumers should reach for `microBase.X` first;
   only add a new function to `microBase` if at least two consumers need it.
2. **No guessing, hard-exit on bad input.** Every error path is
   `print(message, file=sys.stderr); sys.exit(1)` (CLI path) or a Qt signal
   with the message (GUI path). No silent fallbacks, no try/except that
   swallows errors. The one exception is `microProfiler` CLI batch mode:
   a per-dataset `Exception` is logged and the batch continues to the next
   dataset (intentional, so one bad dataset doesn't abort a plate scan).
   `MetadataValidationError` still hard-exits (§3.14).
3. **Explicit over implicit.** `channel_layout` is a constructor argument
   ...
   group, each file is one channel (column name = `ch` + raw capture when
   the capture starts with a digit, e.g. `ch1`, `ch01`; non-numeric
   captures like `blue` stay as-is, e.g. `blue`); if no `channel` group,
   each file is treated as the single implicit channel `ch1` (supports
   non-standard single-channel datasets like HPA). The previous hard-exit
   on `channel_layout=None` without a `channel` group has been relaxed.
...
5. **1-based channel indexing** in the public API (`read_tiff_channels`,
   ...
   starts with a digit: `ch1`, `ch2`, `ch01` (zero-padded preserved).
   Non-numeric captures (e.g. `blue`, `green` from non-standard datasets
   like HPA) are used verbatim as the column name with no `ch` prefix. No
   numeric coercion is applied — the capture is used verbatim.
6. **Config is YAML + manual validation.** No pydantic for top-level
   `PipelineConfig` (it is a dataclass). `pydantic` IS used internally by
   the BaSiC illumination-correction algorithm (`preprocessing/basic/`) for
   its own config validation. Validate `PipelineConfig` by reading keys
   explicitly and `sys.exit(1)` on missing/invalid values.
...
9. **Per-module try/except + print + `sys.exit`.** No central exception
   ...
   `microProfiler.pipeline` (raised in `_run_zproject`/`_run_tile`, caught
   in `cli.main`), documented in §3.14.
10. **No schema versioning for `infer.db`.** Old/dirty DBs must be deleted
    manually — there is no `_meta` table and no migration path.
11. **`NO_ALBUMENTATIONS_UPDATE=1`** and **`ALBUMENTATIONS_NO_TELEMETRY=1`** are
    set in `microBase/__init__.py` (via `os.environ.setdefault`, respecting any
    pre-existing value) before any sub-module import — suppressing
    AlbumentationsX's version-check network calls and telemetry.
...
13. **All metadata is TEXT from extraction to DB storage.**
    ...
    stripping, no int coercion. `'01'` stays `'01'`, `'blue'` stays
    `'blue'`, `'000a6c98-...'` stays as-is. `derive_well` is the only
    exception: it computes well labels (e.g. `'A1'`) from `row`+`col`
    by doing its own `int()` internally. Structural columns
    (`well`, `field`, `stack`, `timepoint`) that are absent from the
    regex are simply missing — they are NOT defaulted to `0`. Missing
    columns must be tolerated by consumers (microVis UI hides empty
    filters; microModel writes only the captured columns). Extra
    regex-captured columns (anything not structural, not `ext`/`tile`/
    `mask_name`, not a path/mask/intensity column) are propagated as
    dynamic TEXT columns through microVis CSV exports and microModel
    `infer.db` (both single-cell and whole-image modes).
14. **Metadata-gated pipeline steps.** Pipeline steps that require a
    specific metadata column must validate at the pipeline level before
    running — no silent skip, no silent fallback. Z-projection requires
    `stack`; tiling requires `field`. CLI path: `print(msg, file=sys.stderr);
    sys.exit(1)` (raised as `MetadataValidationError` in
    `microProfiler.pipeline`, caught in `cli.main`). GUI path: the
    worker's `error` signal fires `on_pipeline_error`, which shows a
    `QMessageBox.critical` pop-up and discards the operation.
15. **Channel fallback.** If `image_pattern` has no `{channel}` group and
    ...
    column `ch1` to every image group. This `ch1` is the default channel
    available to microProfiler segmentation and profiling when no explicit
    channel metadata exists. If a `{channel}` group is present, the `ch`
    prefix is added only when the capture starts with a digit (e.g. `1` →
    `ch1`, `01` → `ch01`); non-numeric captures are used verbatim (e.g.
    `blue` → column `blue`).
16. **session.yml read-once, write-on-action.** `session.yml` is read only
    on the FIRST browse of a given dataset directory (to pre-fill GUI
    fields the GUI needs; missing fields stay at GUI default). Subsequent
    browses of the same directory preserve the user's GUI edits —
    `session.yml` is never re-read into the GUI. "Load Dataset" is an
    action button in both microProfiler and microVis: it writes the user's
    current patterns to `session.yml` via `set_patterns()` before scanning
    data. Other action buttons (Run, Apply, Export, Auto, Write-to-DB,
    etc.) also persist GUI state — step params via
    `_save_session_yml` (microProfiler), channel colors via
    `_persist_channel_colors` (microVis). The only exempt buttons are
    "Browse" (selects directory + reads session.yml only) and full "Reset"
    (Data tab). This applies to both microProfiler and microVis, in all
    panels/steps. For channel colors specifically: on reload, the user's
    current GUI colors are preserved for channels that still exist,
    defaults are used for new channels, and removed channels are dropped —
    `session.yml` `channel_colors` are never re-read on reload.
17. **Modal dataset loading.** Both microProfiler and microVis construct
    ...
    large datasets. The worker (`DatasetLoadWorker` in microProfiler,
    `_DatasetLoadWorker` in microVis) emits `finished`/`error` signals that
    trigger phase-2 UI population (`_on_dataset_loaded`) or an error
    dialog (`_on_dataset_load_error`). The dialog is indeterminate
    (`setRange(0, 0)`) with no cancel button — the user must wait for
    completion.
...
**Version:** `0.5.0`  •  **Layout:** flat, 10 modules under `src/microBase/`
**Dependencies:** `numpy`, `pandas`, `tifffile`, `Pillow`, `scikit-image`,
`scipy`, `pyyaml`, `albumentationsx`, `natsort`.
...
| `schema.py` | Classify regex-captured columns; derive `well` from `row+col`; `normalize_capture` is a verbatim pass-through (no coercion — all metadata is TEXT). | `MetadataSchema`, `derive_well`, `normalize_capture` |
...
| `cells.py` | Pure functions for cropping single cells from labeled masks. | `get_labels`, `crop_cell`, `crop_all_cells` |
...
| `normalize.py` | Per-channel / global / null z-score with percentile clipping. | `normalize` |
...
- `channel_layout=None` ⇒ one-channel-per-file. Two sub-modes:
  - `image_pattern` HAS a `channel` group → each file is one channel;
    ...
    `blue` stays `blue`).
  - `image_pattern` has NO `channel` group → each file is treated as the
    single implicit channel `ch1` (supports non-standard single-channel
    datasets like HPA). A warning is logged.
...
- Properties: `metadata`, `intensity_colnames`, `mask_colnames`, `img_shape`,
  ...
  column names; the `channel` capture is consumed into the intensity-column
  mapping and intensity/mask columns are excluded).
...
- LRU cache size 8, keyed by `row_idx`. On cache miss, all available masks
  are loaded (not just the requested subset) to keep the cache complete.
  The cache is thread-safe (guarded by a lock) — microVis workers access it
  from background threads.
- **Sorting.** `metadata` rows are natsorted by structural columns in
  microscopy-natural priority: `row → col → field → stack → timepoint`
  (columns absent from the regex are skipped; if none exist, insertion
  order is kept). `intensity_colnames` and `mask_colnames` are natsorted.
  `schema.extra_cols` is natsorted. This gives natural order
  (`field=2` before `field=10`, `ch1` → `ch2` → `ch10`) rather than
  lexicographic (`field=10` before `field=2`). Consumers (microProfiler
  dataset info, microVis filter panel) inherit this order automatically.
...
- **Sorting.** Files are iterated in `natsorted` order (e.g. `cell_1.tiff`
  → `cell_2.tiff` → `cell_10.tiff`, not lexicographic `cell_1` → `cell_10`
  → `cell_2`), which determines `metadata` row order.
...
- Hard-exits (`print` to stderr + `sys.exit(1)`) if the cell has zero
  pixels — possible mask corruption.

#### `normalize(image, mask=None, method="per_channel", clip_low=0.05, clip_high=99.95)`

- `method`: `"per_channel"` | `"global"` | `"null"` | `None`.
- Always applies percentile clipping, even for `"null"`.
- Returns `(H, W, C) float32`. Background (mask falsy) is zeroed.
...
- Methods: `exists()`, `load()`, `save(updates)` (deep-merge),
  `get_patterns()`, `set_patterns(...)`, `get_applied_steps()`,
  `set_applied_steps(steps)`, `get_channel_colors()`,
  `set_channel_colors(colors)`.
...
`r1c1f01p01-ch1sk1`). Both microProfiler and microVis import these as
GUI defaults at startup; users override them by typing custom patterns
in the GUI or setting them in `session.yml`.

- `DEFAULT_IMAGE_PATTERN` — matches `r{row}c{col}f{field}p{stack}-ch{channel}(sk|t){timepoint}*.tiff`. The `0?` prefix on `field`/`stack`/`channel`/`timepoint` makes a single leading zero optional (consumed before the capture group), while `row`/`col` capture all digits verbatim.
...
**Version:** `1.3.0`  •  **Entry:** `microprofiler` (CLI: `run`; GUI: no args)
...
├── config.py              PipelineConfig dataclass + YAML load/save
├── pipeline.py            run_step / run_pipeline orchestration
├── logging_utils.py       logger factory
├── progress_collector.py  pub-sub progress (CLI tqdm + GUI signals)
├── io/
│   ├── __init__.py        re-exports microBase io + thin wrappers
...
│   ├── z_projection.py    max/mean/min Z collapse
...
│   ├── main_window.py     top-level QMainWindow, 4-page nav
...
│   ├── panels/            BaseStepPanel + BlockContainerPanel + 7 step panels + FilterPanel
...
```
                ...
                └────┬────┘            image_subdir_pattern, filters)
                     ...
                     ▼
   ┌─────────────────────────────────────────────────────┐
   │  resize → basic → zproject → tile → segment → profile  │
   └─────────────────────────────────────────────────────┘
                     │
        ┌────────────┴────────────┐
        ▼                         ▼
...
- The four preprocessing steps (`resize`, `basic`, `zproject`, `tile`) are
  ...
  steps. `segment` and `profile` always run (not gated by `applied_steps`) —
  they are non-destructive and intended to re-run (e.g. after adding a new
  object profiling config entry). **`applied_steps` never shrinks**: the GUI
  persists the union of the previous applied steps and the steps that
  actually executed (`PipelineController._save_session_yml`), so unchecking
  a checkbox can never cause a destructive re-run (double resize/zproject/
  tile) of already-processed files.
...
- **Profiling failure semantics.** Per-row and per-channel feature failures
  ...
  run). A run-level profiling failure, however, is re-raised: batches
  flushed before the failure are already committed, but the dataset is NOT
  treated as complete — the CLI batch loop logs the dataset failure and
  continues, the GUI worker surfaces the error dialog, and
  `cli._is_dataset_complete` additionally requires the `image` table to
  contain one row per (filtered) image, not merely to exist. Object tables
  have no row-count guard — after a failed object run, delete `result.db`
  (or the affected table) before re-running.
- **Config-driven overwrite flags (no CLI flags).** There are no
  `--overwrite-db` / `--overwrite-mask` CLI flags — specific parameters are
  passed via config or GUI. Per-entry overwrite control lives in the config
  dataclasses and is persisted to `session.yml`:
  - `segment.configs[].overwrite_mask` (default `false`) — when `True`,
    ...
    already exists. Set via the "Overwrite mask" checkbox in the Segmentation
    panel.
  - `object_profile.configs[].overwrite_db` (default `false`) — when `True`,
    `_run_profile` drops the existing `<output_table_name>` table from
    `result.db` before profiling; when `False`, the `BatchWriter` replaces the
    table on first write anyway. Set via the "Overwrite DB" checkbox in the
    Object Profiling panel (same row as "Output table").
  Both checkbox states are serialized by `build_config_section` and restored
  by `_apply_block_config` / `_load_block_from_stored`.
- **Dataset-level skip (CLI only).** `cli.main` calls `_is_dataset_complete`
  ...
  (filtered) dataset size — the dataset is skipped. The row-count check
  scans the dataset directory (metadata build), which is the dominant cost
  of the skip check on large plates. To re-process a skipped dataset,
  delete `result.db` (or the specific tables). There is no flag to bypass
  this check or to force re-running already-applied preprocessing steps —
  delete `session.yml` or edit `applied_steps` manually.
...
- **Pre-flight metadata validation.** `zproject` requires a `stack` column
  ...
  raised in `pipeline._run_zproject`/`_run_tile` before any work begins.
  The CLI catches it and hard-exits (`sys.exit(1)`); the GUI worker emits
  `error`, which `on_pipeline_error` surfaces as a `QMessageBox.critical`
  pop-up (see §3.14).
- **session.yml read-once, write-on-action** (see §3.16). The GUI "Browse"
  button reads `session.yml` only on the first browse of a given directory
  (pre-fills patterns + step params + applied_steps + filter); subsequent
  browses of the same directory preserve the user's GUI edits. Manually
  editing the input/output directory fields invalidates the loaded dataset
  (a Run can never process stale files into a different directory). "Load
  Dataset" is an action button: it writes the user's current patterns to
  `session.yml` via `SessionFile.set_patterns()` before scanning data.
  Other action buttons (Run Preprocessing, Run Segmentation, Run Profiling,
  Apply) persist full GUI state (step params + patterns + filter +
  applied_steps) via `PipelineController._save_session_yml`.
- **Cooperative cancel.** `PipelineWorker`/`PreviewWorker` observe a cancel
  ...
  pipeline/GUI threads are never terminated mid-execution (no
  `QThread.terminate`). Cancelling just stops the run at the next checkpoint.
  Note: cancelling mid-run still unions the checked steps into
  `applied_steps` (the finished path persists the checkbox state) —
  intentionally. This prevents a later re-run from re-applying an in-place
  step (`resize`/`basic`/`zproject`/`tile`) onto half-processed files. After
  an interrupted in-place step, restore the original raw data and clear
  `applied_steps` (`session.yml`) before re-running.
...
filter:               [{column: well, pattern: "A[1-6]"}]
...
segment:
  ...
  configs:
    - object_name: cell
      model_name: cpdino
      chan1: [ch1]            # list of channel names
      chan2: [ch2]            # optional, null or list
...
4. Add a `XxxStepPanel(BaseStepPanel)` under `gui/panels/` with a `_FIELD_MAP`
   ...
   (`to_config`/`from_config`); block-list panels (segment/profile) override
   `build_config_section` and use the structured `load_config_section`
   list-of-dicts format via `BlockContainerPanel`.
...
**Version:** `1.1.0`  •  **Entry:** `microvis` (GUI only — no CLI subcommands)
...
├── worker.py              QRunnable workers (image/full-res/crop/export)
├── io/
...
- `MainWindow` owns one `DataModule`, one `QThreadPool`, and the label
  annotation state.
- Workers are `QRunnable`; they emit Qt signals back to `MainWindow`.
...
- **session.yml read-once, write-on-action** (see §3.16). "Select Dataset
  ...
  Re-selecting the same directory preserves GUI edits. Browsing a
  different directory clears old display state (images, grid, caches,
  label panel). "Load Dataset" (renamed from "Reload Dataset") is an
  action button that writes patterns + channel_colors to `session.yml`
  before starting the data scan. CLI launch (`microvis <dir>`) pre-fills
  the directory + patterns but defers scanning to "Load Dataset". Export,
  Write-to-DB, Auto, and ImageControls Reset also persist GUI state. On
...
|---|---|---|
| `<dataset>/session.yml` | read + write | Read once on first Browse of a directory (`get_patterns`, `get_channel_colors`); written by action buttons — Load Dataset, Export, Write-to-DB, Auto, ImageControls Reset (`set_patterns`, `set_channel_colors`). See §3.16. |
| `<dataset>/result.db` | read + write | Reads profiling tables; writes merged metadata tables (`write_merged_table`) and label annotation tables (`write_label_table`, default name `{mask}_label`, columns `well, field, stack, timepoint, label, class`) |
...
- TIFF axes: `CYX`, with `Labels`/`LUTs`/`mode='composite'` metadata for
  ...
  ramp) encode the channel colors chosen in the channel-control panel so
  Fiji tints each channel accordingly.
- Cropping uses `microBase.ImageDataset.get_cropped_cells(row_idx, mask_col,
  ...
  crops are pixel-identical. Rows whose mask is missing (e.g. Cellpose found
  zero cells) are skipped with a warning — the export continues and reports
  the skipped count. A run with zero enabled channels errors out, and a CSV
  manifest write failure is reported as an export error (objects are never
  reported as exported without their manifest).
...
its `WellGridControls` are hidden by setting the top splitter container
(`self._well_grid_container`) invisible in `_on_dataset_loaded`. Image
selection then depends solely on the Image Filter panel (field/stack/
timepoint/extra cols, top-100 natsorted per filter). When wells exist, the grid is
shown and is the primary well-selection mechanism (`_selected_wells`).
...
box to find. `_ClassBox` holds a `self._panel` reference (passed in
`LabelAnnotationPanel.add_class`) to iterate sibling boxes.
...
the metadata, using the conventional format `A1 f1 z1 t1`. If no
structural columns exist, it falls back to extra cols as `col=val` pairs,
then to `row {row_idx}`. Worker results carry `meta_label` through to the
`QLabel` above each thumbnail. Sort comparison in `_add_thumbnail_sorted`
reads `thumb._well/_field/_stack/_timepoint` attributes directly (not by
parsing label text).
...
`_populate_label_controls`. Default is `None`. The choice is ephemeral — it is
NOT persisted to session.yml (consistent with `overlay_col`/`overlay_cmap`/
`overlay_alpha`, which are also ephemeral) and resets to `None` on every app
start and on `_on_full_reset`.
...
- **`None`** → no overlay whatsoever: no polygons, no hover tooltip, no
  ...
  the image view (the original user complaint).
...
When `Color by` is None → outline only (1px gold, no fill). When `Color by` is
also set → fill polygons by Color-by value + gold outline (`outline_only=False`).
`Color by` alone (with `Select object` = None) renders nothing — Color by only
takes effect once a mask is selected.
...
mask. `ImageWorkerConfig` and
`FullResWorker` carry `selected_mask_name` + `outline_only`; the latter bakes
`outline_only` into the full-res pixmap (the thumbnail's `_outline_only` is
already set from the `ImageWorker` result, so `set_full_res_pixmap` needs no
`outline_only` argument).
...
Export panel's separate `Object` dropdown (`_export_mask_combo`, populated via
`set_label_masks` / read via `get_export_mask`) is independent and controls
only the export flow — it is unaffected by `Select object`.
...
`microModel` is self-contained — it does not import from the other
consumers. Common helpers (`build_backbone`, `FocalLoss`,
`ClassificationHead`, `Model`, `SingleCellDataset`, `WholeImageCellDataset`,
utils) are duplicated into `microModel`. SSL pretraining is its main
addition over plain classification training; train + infer cover the
classification/feature-extraction surface.
...
pretrain_ssl(config)
   ...
   │
   ├─► subsample(pairs, sample_max, sample_by, seed)
...
`train_step` uses exactly 2 views (`views[0:2]`); extra views are ignored.
Momentum schedule is cosine `momentum_start` → `momentum_end` (default
0.996 → 1.0) over `total_steps` (step-based, config-driven). The momentum EMA
update itself runs once per optimizer step, right after `optimizer.step()`
(the canonical BYOL/DINO timing — teacher lags the student). LR schedule:
linear warmup (0 → `training.lr` over `warmup_epochs`) then cosine decay
(`training.lr` → `lr_final`). `transfer_warmup_epochs` (default 1) is used
instead of `warmup_epochs` in transfer resume mode. Loss:
`NegativeCosineSimilarity`. Note: lightly 1.5.x's `NegativeCosineSimilarity`
returns `-cosine_similarity(...)` (no `1 -`), so the loss range is `[-1, 1]`
and a DECREASING (more negative) loss means the views are becoming more
aligned — `-0.95` ≈ cosine 0.95. Negative losses are expected, not a bug.

**AMP hardening (pretrain)**: with `amp: true`, the BYOL MLP heads
(`forward` / `forward_momentum`) run in fp32 — the backbone flatten output is
cast to `float` and the heads execute under `torch.amp.autocast(enabled=False)`.
This fixes rare all-NaN head outputs on some GPUs (cuBLAS fp16 matmul
reduced-precision accumulation overflow, `allow_fp16_reduced_precision_reduction`),
where the same input+weights are clean in fp32 but NaN in fp16. Additionally
`pretrain_ssl` sets
...
batch = `batch_size * K` at ~1/K of the optimizer-step memory. `train_step`
(both methods) takes a `step=True` kwarg: backward runs every micro-batch;
...
when `step=True`. `pretrain_ssl` computes `do_step` from the micro-batch index
(step every K-th batch and at the epoch tail, since `drop_last=True` leaves
a partial window that must flush). BN batch statistics still use the
micro-batch size — expected for BYOL/DINOv2 with accumulation.
...
`vit_large_patch14_224`); non-ViTs and `gap`/no-cls variants hard-exit with a
clear message. The teacher is frozen. `backbone.pretrained` is honored:
`pretrained=True` loads ImageNet weights
into teacher + student (timm adapts the first conv to the `data.channels`
count via `adapt_input_conv`; the classifier is dropped and `strict=False`
keeps the missing `layer_scale` params at their `1e-5` init). The lightly
wrapper is
constructed with `weight_initialization="skip"` when `pretrained=True` —
lightly's default MAE-style `_initialize_weights()` would otherwise overwrite
the loaded weights. `backbone.in_chans` is NOT a config key — the input
channel count is derived from the resolved `data.channels` (and from the
bundle meta on resume; a mismatch hard-exits). `dinov2.input_dim` is optional
(`null` = derived from the
ViT's `embed_dim`); a non-null value must match or pretrain hard-exits.
`augmentation_views[0:2]` are global views (2 required),
`views[2:]` are local views (any count: 1, 2, 4, 6, 8, ...). The student
receives masked global views + all local views; the teacher receives
unmasked global views. Uses `MaskedVisionTransformerTIMM` from lightly.
Losses: `DINOLoss` (cls token) + `IBOTPatchLoss` (masked patches) +
`KoLeoLoss` (student cls token of global views). Teacher temperature, weight
decay, and momentum are cosine-scheduled over
`total_steps = epochs * len(loader)` (the full run). In `continue` mode
`global_step = start_epoch * len(loader)` keeps the absolute step position,
so the schedules extend seamlessly where the interrupted run left off; in
`transfer` mode `global_step` starts at 0 and the schedules restart from
their start values. `warmup_epochs` controls BOTH the teacher temperature
warmup AND the LR warmup (linear 0 → `training.lr`, then cosine decay →
`lr_final`). `transfer_warmup_epochs` (default 1) replaces `warmup_epochs`
in transfer resume mode. `train_step`
returns `(loss, dino, ibot, koleo)` and `train_step`'s per-epoch log line
includes the three components (`epoch=N loss=... (dino=... ibot=... koleo=...)`,
DINOv2 only; BYOL logs the plain total). `train_step`
moves views to the device explicitly (like BYOL). Global views are 224×224
(whole cell, fit+pad) and local views 112×112 (RandomResizedCrop sub-regions) —
mixed sizes are supported via `dynamic_img_size`; only views in the same
group (`views[0:2]` vs `views[2:]`) must share one size, since each group is
`torch.cat`-ed. Every view's size must be divisible by the backbone's patch
size (timm hard-asserts this) — 224 and 112 work for patch 8/14/16 models;
the patch mask is built from the ACTUAL input grid (not the ViT's default
img_size, which is 518 for the `*_dinov2` timm variants), with masking
starting after all prefix tokens (cls + register tokens).

**Head collapse guard**: with fresh random projection heads and a low teacher
temperature, the teacher targets are unfittable noise — the student head's
output variance can collapse toward zero, making both softmaxes uniform and
the DINO/iBOT gradients vanish (the loss locks at the uniform-CE floor,
`dino = ln(65536)`). The hard guarantee is `train_step`'s collapse guard: it
tracks an EMA of the student head's logit spread (std across the 65536 code
dims) and, when it drops below `5e-3`, re-initializes the projection heads
(`DINOv2.reinit_heads` — student fresh, teacher deepcopy, 200-step cooldown)
so the gradient signal returns instead of deadlocking. Two details keep the
re-init correct: `train_step` re-points the optimizer's param groups at the
model's current parameters (the optimizer was created with the pre-reinit
heads, which would otherwise be orphaned — the fresh heads would never be
trained), and the projection-head forwards run in fp32 (`autocast off` —
fp16 matmuls in the 65536-dim heads produced rare NaN on some GPUs, the same
cuBLAS issue that motivated BYOL's fp32 heads; a fresh head's weight
distribution can hit it). The default config
keeps the paper temperatures (`teacher_temp_start/end` 0.04/0.07) and
`momentum_start` 0.992 — a sharp teacher is what drives fast early learning
(~0.3–0.5/epoch loss drop); the guard makes the sharp regime safe.
`dinov2.head_batch_norm` (BatchNorm inside the heads, original DINO design)
is on by default (code default `True`, shipped config `True`) — it normalizes
the head output scale and is a critical anti-collapse mechanism: without it,
the 65536-dim head outputs collapse to a constant (uniform softmax) and the
loss locks at the uniform-CE floor (`log(65536)` ≈ 11.09) forever. The
teacher's BN buffers are kept in sync via the buffer-aware momentum update,
...
- **`continue`** — exact same-data extension: the full model state (incl.
  ...
  and loss history are restored; the run continues where it left off.
- **`transfer`** — domain transfer / pretrained-weight init (e.g. new data):
  ...
  momentum restarting at its start value) and a fresh optimizer (AdamW/SGD
  moments are stale for the new data). Loss history is reset.
Locked keys (bundle wins on mismatch) — `method`, `backbone.name`,
`augmentation_views`, the `byol:`/`dinov2:` method block, `normalize.*`.
`backbone.in_chans` is not locked (no longer a config key): on resume the
bundle's `meta.in_chans` is used, and a mismatch with the resolved
`data.channels` hard-exits. Unknown `resume.type` values hard-exit.

**UMAP check (`training.n_image_umap`, pretrain only)**: optional per-interval
...
train(config)
   ...
   │
   ├─► resolve labels (label_from_dir | label_csv)
   ├─► subsample(records, sample_max, sample_by, seed)
   ├─► stratified_split(records, val_ratio, seed)
   │
...
the ViT is rebuilt via `backbone.build_dinov2_vit` (the shared ViT factory)
with `pos_embed="learn", dynamic_img_size=True, init_values=1e-5` and
**cls-token pooling** (`x[:, 0]`). For BYOL or conv backbones, `build_backbone`
is used with global mean pooling. This logic lives in
`train._build_model_from_ssl`, `backbone.load_model_from_bundle`, and
`backbone.load_ssl_backbone_from_bundle`. The final eval-model rebuild (and
the saved `model.pt` meta `backbone`/`in_chans`) uses the backbone that was
ACTUALLY trained — the SSL bundle's `meta.backbone`/`meta.in_chans` when
transferring, NOT the train config's `model.backbone` — so a config/bundle
mismatch can't silently build the wrong architecture.

**Resume** (`resume.sl_model`): may point at `model.pt` or any
`model_{epoch}.pt` (interval bundles exist only when `save_interval` is hit;
`model.pt` is always written). Locked keys — `model.backbone`,
`model.pretrained`, `model.focal_gamma`, `augmentation_train`,
`augmentation_infer`, `normalize.*`. Fallback keys (use new config if
present, else bundle): `training.epochs`, `training.lr`,
`training.weight_decay`, `training.betas`, `training.patience`.
`resume.sl_model` and `resume.ssl_model` are mutually exclusive — if both are
set, `train` prints an error and hard-exits before any data loading.
When `resume.sl_model` is set, the model architecture and weights come entirely
from the checkpoint: the backbone is built with `pretrained=False` (the
checkpoint state dict would fully overwrite any timm pretrained load), and
`data.channels` is validated against the checkpoint's input channels
...
run_inference(config)
   ...
   │     • feature (default true): write the features BLOB (was output_features).
   │
...
fit-once-on-merged-DBs then applied per-DB.
...
| Inference DB (per-dataset) | `<dataset>/infer.db` |
