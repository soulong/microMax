...
class BaSiCStepPanel(BaseStepPanel):
    ...
    def set_preview_channels(self, channel_names):
        ...
        for ch in channel_names:
            ...
            # Channel previews are wide but SHORT (half the previous height,
            # aspect ratio preserved by the viewer's KeepAspectRatio fit).
            raw_tile = ChannelTile("raw", np.zeros((64, 64)))
            raw_tile._viewer.setMinimumSize(dp(200), dp(100))
            raw_tile._viewer.setMaximumHeight(dp(100))
            corr_tile = ChannelTile("corrected", np.zeros((64, 64)))
            corr_tile._viewer.setMinimumSize(dp(200), dp(100))
            corr_tile._viewer.setMaximumHeight(dp(100))
            ff_tile = ChannelTile("flatfield", np.zeros((64, 64)))
            ff_tile._viewer.setMinimumSize(dp(200), dp(100))
            ff_tile._viewer.setMaximumHeight(dp(100))
